...
Arbeidsledighetsprosenten blir beregnet som: arbl% = antall arb. ledige/arbeidsstyrken. Arbeidsstyrken er her dem man anser egnet for arbeid dvs. uføretrygdede etc. er trukket ut. Arbeidsstyrken i en alderskategori er derfor langt mindre enn befolkningen i den tilsvarende alderskategorien.
...
```{r}
# Setter farge utenfor aes(). Setter vi den inne i aes()
# vil vi bruke farge til automatisk å skille ulike kategorier. 
# Da måtte vi hatt en kategorivariabel (cat_wf) som spesifiserte om 
# det var snakk om relativ wf for hhv. kvinner, menn eller k+m
# Da ville vi brukt aes(x = år, y = wf_K/bef_K, colour = cat_wf)
# ggplot ville da automagisk ha satt fargene for oss.
wf %>% 
...
Forklar kort den generelle utviklingen i arbeidsstyrken. Hva er det som «redder oss»?

*Over tid ser vi at menn generelt har har hatt en synkende trend (sett bort fra vestlandet som har hatt en økende trend). På den andre siden har kvinner hatt en generelt økende trend. Og samlet sett ser man vekts over tid, men samtdig en liten nedgang de siste årene*
...
Skriv en funksjon som kjører den lineære modellen wf_KM ~ alp_15_29 + alp_30_74 på en input dataframe a_df. Kall funksjonen mod1 (i mangel på noe bedre navn).
