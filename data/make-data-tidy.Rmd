...
u_knavn <- unique(arblos_aldersgrp$knavn)
head(u_knavn[str_detect(u_knavn, "\\(")], n = 10)
```
Som en ser over ender en opp med mange navn som ikke er fikset. Løsning: Istedenfor å ta spesialtilfeller spesifiserer vi et generelt mønster vha. en «regular expression».

Starter med 1 eller flere «white space»: `\\s+`
så kommer én parentes-start: `\\s+\\({1}`
så kommer tall eller bindestrek `\\s+\\({1}[-\\d]+`
helt opp til én parentes-slutt: `\\s+\\({1}[-\\d]+\\)`
...
# Nå er alle vekk, beholder (fylke) siden dette skiller
# der vi har to kommuner med samme navn
u_knavn <- unique(arblos_aldersgrp$knavn)
head(u_knavn[str_detect(u_knavn, "\\(")], n = 10)
```
...
u_knavn <- unique(df_arbl_p_kjonn$knavn )
head(u_knavn[str_detect(u_knavn, "\\(")], n = 10)
```
...
u_knavn <- unique(bef9914MK$knavn)
head(u_knavn[str_detect(u_knavn, "\\(")], n = 10)
```
